public class LeasingCost {
     ...
     */
	public static Vehicle createVehicle(String filename){
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for (int i = 0; i < vehicles.length; i++) {
            ...
            double months = tempVehicle.getLease().getLeaseLength();
            double usage = tempVehicle.getFuel().getUsage();
            double mileageAllowance = tempVehicle.getLease().getMileageAllowance();
            double co2Punit = 0;
            double due = tempVehicle.getLease().getDueAtSigning();
            double monthlyCost = tempVehicle.getLease().getMonthlyCost();
            double leaseCost = computeLeaseCost(due, (int) months, monthlyCost);
            if (tempVehicle.getFuel().getType() == 1) {
                co2Punit = Fuel.CO2EMITTED_GASCOMBUSTION;
                tempVehicle.setFuelCost(computeFuelCost(months, usage, mileageAllowance, gasPrice));
            } else {
                co2Punit = Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION;
                tempVehicle.setFuelCost(computeFuelCost(months, usage, mileageAllowance, electricityPrice));
            }
            double total = tempVehicle.getFuelCost() + leaseCost;
            tempVehicle.setTotalCost(total);
            tempVehicle.setCO2Emission(computeCO2(months, usage, mileageAllowance, co2Punit));
        }
    }
